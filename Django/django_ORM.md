...
## HTTP requsert methods
네트워크 상에서 데이터를 주고 받기위한 약속     
HTTP requsert methods: 데이터에 대해 수행을 원하는 작업을 나타내는 것     
### GET Method
검색 쿼리 전송, 웹 페이지 요청, 그리고 API에서 데이터를 조회하는 것과 같이
서버로부터 데이터를 요청하고 받아는에 사용   

DB 변경 X -> 데이터 요청만.  

### POST method  
로그인 정보, 파일 업로드, 데이터 생성, API에서 데이터 변경을 요청하는 것과 같이 
클라이언트가 서버로 데이터를 전송하여 서버의 상태를 변경할 때 사용됩니다.     
...
클라이언트는 이 코드를 통해 요청이 성공, 실패 했는지 아니면 추가적인 조치가 필요한지 즉시 파악 가능    


### CSRF  
사이트간 요청 위조  
사용자의 의지와 무관하게 공격자가 의도한 행동(글쓰기, 송금, 수정 등)을 요청하게 만드는 해킹 방식   

방어하기 위해 CSRF 토큰을 사용해 안전장치 마련

## Redirect 
클라이언트가 인자에 작성된 주소로 다시요청을 보내도록 하는 함수

return render 대신 return redirect('articles:detail', article.pk)